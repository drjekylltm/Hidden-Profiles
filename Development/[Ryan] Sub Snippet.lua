...
A[3] = function(icon) 
    ...
    function EnemyRotation(unitID) 
        ...
        local function CDs()
            ...
            print("--SYMBOLS--")
            print("RDY", A.SymbolsOfDeath:IsReady(player) )
            print("AND", SnD_Condition())
            print("AND", (A.EnvelopingShadows:IsTalentLearned() or A.ShadowDance:GetSpellChargesFrac() >=1)) 
            print("AND", (not A.ShurikenTornado:IsTalentLearned() or A.ShadowFocus:IsTalentLearned() or A.ShurikenTornado:GetCooldown()>2))
            print("AND", (Unit(unitID):TimeToDie() > 10 or Unit(unitID):IsBoss()))
            print("AND", (Unit(unitID):HasDeBuffs(A.Rupture.ID, true) >= 8 or skip_rupture))
            print("AND", (Unit(unitID):HasDeBuffs(A.Rupture.ID, true) >= 8 or skip_rupture))
            print("AND", inMelee)
            ...
            (
                ...
                and (
                    A.SymbolsOfDeath:GetCooldown() < 1 
                    or 
                    (Unit(player):HasBuffs(A.SymbolsOfDeath.ID) > 7)
                )
                or (
                    Unit(unitID):IsBoss() 
                    and 
                    Unit(unitID):TimeToDie() <= 21
                )
            )
            then
        ...
        local function StealthCooldowns()
            ...
            --print("--- FIRST COND --- ", (shd_combo_points
            --and A.ShadowDance:IsReady(player)
            --and inMelee
            --and GetCurrentGCD() <= 0.1
            --and A.ShadowDance:GetSpellChargesFrac() >=1
            --and (isBurst or shd_threshold - (not A.EnvelopingShadows:IsTalentLearned() and 0.75 or 0))))
            
            --print("RDY", A.ShadowDance:IsReady(player))
            --print("SHD_THRES", shd_threshold)
            --print("ENERGY_THRES", (Player:EnergyDeficit() <= stealth_threshold))
            --print("SHD_COMBO", shd_combo_points)
            --print("GCD", GetCurrentGCD() <= 0.1)
            --print("CHARGES", A.ShadowDance:GetSpellChargesFrac() >=1)
            --print("WEIRD", (isBurst or shd_threshold - (not A.EnvelopingShadows:IsTalentLearned() and 0.75 or 0)))
            ...
            if (shd_combo_points
                ...
                --and GetCurrentGCD() == 0
                and A.ShadowDance:GetSpellChargesFrac() >=1
                and (isBurst or shd_threshold - (not A.EnvelopingShadows:IsTalentLearned() and 0.75 or 0))) then
                
                
                --print("----SECOND COND----", (shd_threshold or Unit(player):HasBuffs(A.SymbolsOfDeath.ID) >=1.2 or Unit(player):HasBuffs(A.ChaosBaneBuff.ID) > 0
                ...
                --print("SHD_THRES", shd_threshold)
                --print("Symbols 1", Unit(player):HasBuffs(A.SymbolsOfDeath.ID) >=1.2)
                --print("Symbols 2", A.SymbolsOfDeath:GetCooldown() > 10)
                --print("PV", (Unit(player):HasBuffsStacks(A.PerforatedVeins.ID) <= 4))
        ...
        local function Opener()
            ...
            --print(isDoneOpener)
            -- Kyrian Opener
            if Player:GetCovenant() == 1 and isBurst and not isDoneOpener and Unit(player):CombatTime() <= 15 and inCombat then
                ...
                and
                Unit(player):HasBuffs(A.Stealth.ID) ~= 0 then
                    return A.Shadowstrike:Show(icon)
                end
                
                if  A.Shadowstrike:GetSpellCounter() == 1 and Player:ComboPoints() == 2 and Unit(player):HasBuffs(A.SliceAndDice.ID) ~= 0 then
                    return A.PoolResource:Show(icon)
                end
                
                if A.SliceAndDice:IsReady(unitID) and
                A.Shadowstrike:GetSpellCounter() == 1 and
                Player:ComboPoints() >= 3 and
                (Unit(player):HasBuffs(A.SliceAndDice.ID) ~= 0 or MultiUnits:GetByRange(10) >=6) then
                    return A.SliceAndDice:Show(icon)
                end
                
                if (Unit(player):HasBuffs(A.SliceAndDice.ID) ~= 0 or MultiUnits:GetByRange(10) >=6)
                and A.EchoingReprimand:GetSpellCounter() == 0
                and A.SymbolsOfDeath:GetSpellCounter() == 0
                and inMelee then
                    print( " Pick Pocket aka ER + Symbols + Blades" )
                ...
                and A.SymbolsOfDeath:GetSpellCounter() > 0
                and inMelee
                and A.Rupture:IsReady(unitID) and Unit(unitID):HasDeBuffs(A.Rupture.ID, true) < 8 and not skip_rupture then
                    return A.Rupture:Show(icon)
                end
                
                if A.ShadowDance:IsReady(unitID) and (Player:EnergyDeficit() <= 70) and GetCurrentGCD() == 0 and inMelee and Unit(unitID):HasDeBuffs(A.Rupture.ID, true) > 8 then
                    print( " Dance + SS " )
                    return A.ShadowDance:Show(icon)
                end
                
                if ((A.ShadowBlades:GetSpellCounter() > 0) and
                    (A.EchoingReprimand:GetSpellCounter() > 0) and
                    (A.SymbolsOfDeath:GetSpellCounter() > 0)) or
                Unit(player):CombatTime() <= 15 then
                    print( " Opener Kyrian end" )
                    isDoneOpener = true
                end
                
                if not isDoneOpener then
                    return
                end
            end
        end
        local function Finishers()
            -- TODO: Should not look only at target but at all enemies in 10yd range
            if(A.SliceAndDice:IsReady(player) and Unit("target"):IsBoss() and Unit(player):HasBuffs(A.SliceAndDice.ID) < Unit(unitID):TimeToDie()) then
            ...
            if A.Rupture:IsReady(unitID) and Unit(player):HasBuffs(A.RupturingSpike.ID) == 0 and
            (
