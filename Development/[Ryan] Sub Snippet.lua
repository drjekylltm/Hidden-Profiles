...
local function ValidAutotarget(nameplate)
    -- @return boolean
    --This function checks the KeepTarget table for targets you should not swap to for any reason
    local npc_id = getNPCID(nameplate)
...
local function HasEchoingBuffs() 
    return Unit(player):HasBuffs(323558,_,true) > 0
    or Unit(player):HasBuffs(323559,_,true) > 0
    or Unit(player):HasBuffs(323560,_,true) > 0
    or Unit(player):HasBuffs(354838,_,true) > 0
end
...
local function ComboPointsDeficitEx()
    if(Player:GetCovenant() ~= 1) or not HasEchoingBuffs() then
        return Player:ComboPointsDeficit()
    else
        return EchoingDeficit()
    end
end
...
local function Skip_Rupture()
    ...
    return not ShouldUseDamageOverTime("target") or (A.Finality:HasLegendaryCraftingPower() and Player:FinalityBuffed(UnitGUID("target"))) or (Unit(player):HasBuffs(A.MasterAssassinsMark.ID) ~= 0 or not A.Nightstalker:IsTalentLearned() and A.DarkShadow:IsTalentLearned() and (Unit(player):HasBuffs(A.ShadowDance.ID) ~= 0 or Unit(player):HasBuffs(A.ShadowBlades.ID) ~= 0) or MultiUnits:GetByRange(10) >= 4)
end
...
A[3] = function(icon) 
    ...
    local npc_id = getNPCID("target") --@number npcID from wowhead/evlui, better than using strings for other languages
    ...
    function EnemyRotation(unitID)
        ...
        local npc_id = getNPCID(unitID) --@number npcID from wowhead/evlui, better than using strings for other languages
        ...
        local function StealthCooldowns()
            ...
            if (shd_combo_points
                ...
                and (isBurst or A.ShadowDance:GetSpellChargesFrac() >= (shd_threshold - (not A.EnvelopingShadows:IsTalentLearned() and 0.75 or 0)))) then
                    ...
                    then
                        if (GetCurrentGCD() > 0) and not (effective_combo_points>=Player:ComboPointsMax()) and (GetToggle(2, "AoE") and MultiUnits:GetByRange(10) <3)
                        then
                            return A.PoolResource:Show(icon)
                        else
                            return A.ShadowDance:Show(icon)
            ...
            if (isBurst and Unit("target"):IsBoss() and Unit(unitID):TimeToDie() < A.SymbolsOfDeath:GetCooldown()) then
                if (GetCurrentGCD() > 0) and not (effective_combo_points>=Player:ComboPointsMax()) and (GetToggle(2, "AoE") and MultiUnits:GetByRange(10) <3)
                then
                    return A.PoolResource:Show(icon)
                else
        ...
        local function Opener()
            ...
            if A.ShroudOfConcealment:IsReady(player) and IsInRaid() and (BossMods:GetPullTimer() > 5 and BossMods:GetPullTimer() <= 10) then
        ...
        local function Finishers()
            ...
            and not Unit(unitID):PT(A.Rupture.ID, true) -- current unit is not in Pandemic window
        ...
        local function StealthedRotation()
            ...
            then
                for val in pairs(ActiveUnitPlates) do
                    if     (Unit(val):HasDeBuffs(A.FindWeakness.ID, true) <= 1 --Checck FindWeakness
        ...
        local function Builders()
            ...
            and inMelee
            and not ((EchoingDeficit() == 1 or Player:ComboPointsDeficit() == 1) 
            and Player:SwingCounter() >= 2 
            and Player:Energy()-35 < 27)
            then
                return A.Backstab:Show(icon)
            end
            ...
            and inMelee
            and not (Player:GetCovenant() == 1 and A.ResoundingClarity:HasLegendaryCraftingPower() and (EchoingDeficit() == 1 or Player:ComboPointsDeficit() == 1) and Player:SwingCounter() >= 2 and Player:Energy()-35 < 27)
            then
                return A.Backstab:Show(icon)
            end
            ...
            if A.ResoundingClarity:HasLegendaryCraftingPower() 
            and Player:GetCovenant() == 1
            and ((EchoingDeficit() == 1 or Player:ComboPointsDeficit() == 1) and Player:SwingCounter() >= 2 and Player:Energy()-35 < 27) then 
                return A.PoolResource:Show(icon)
            end
            return false
        end
