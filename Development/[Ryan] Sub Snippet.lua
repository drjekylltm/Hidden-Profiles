...
A[3] = function(icon) 
    ...
    function EnemyRotation(unitID) 
        ...
        local function StealthCooldowns()
            ...
            and Player:Energy() >= 25 
            and GetCurrentGCD() == 0
            and inMelee
            and (Unit(unitID):TimeToDie() > 8 or Unit(unitID):IsBoss())
            and ((Unit(unitID):HasDeBuffs(A.Rupture.ID, true) >= 8) or skip_rupture)
            and A.ShadowDance:GetSpellChargesFrac() >=1
            and shd_combo_points
            and
            (
                (
                    (shd_threshold and Unit(player):CombatTime() >= .8 ) 
                    or Unit(player):HasBuffs(A.SymbolsOfDeath.ID) >=1.2 
                    or Unit(player):HasBuffs(A.ChaosBaneBuff.ID) > 0
                    or (MultiUnits:GetByRange(10) >= 4 and A.SymbolsOfDeath:GetCooldown() > 1)
                )
        ...
        local function Finishers()
            ...
            -- actions.finish+=/variable,name=skip_rupture,value=master_assassin_remains>0|!talent.nightstalker.enabled&talent.dark_shadow.enabled&buff.shadow_dance.up|spell_targets.shuriken_storm>=4", "Helper Variable for Rupture. Skip during Master Assassin or during Dance with Dark and no Nightstalker." );
            local skip_rupture = 
            (Unit(player):HasBuffs(A.MasterAssassinsMark.ID) ~= 0 
                or  not A.Nightstalker:IsTalentLearned() 
                and A.DarkShadow:IsTalentLearned()
